...
# --- DATABASE & FILE PATH STABILIZATION ---
# Using absolute paths ensures Render finds your .db and .html files every time
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
db_path = os.path.join(BASE_DIR, "apt_radar.db")
DATABASE_URL = f"sqlite:///{db_path}"

# Force creation of the database file if it doesn't exist to prevent 500 errors
...
class ScanLog(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    username = Column(String) # Track by username for easier history retrieval
...
# CORS is essential for the frontend to communicate with the backend on Render
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
def scan_url(url: str, user: str, db: Session = Depends(get_db)):
    ...
    if response.status_code == 200:
        ...
        db.commit()
        return data
    return {"message": "Scan initiated or URL check failed."}
